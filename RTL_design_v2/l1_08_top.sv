...
module l1_08_v2_top #(
    ...
    logic buffer_clear;
    logic buffer_wr_clear_meta;
    logic buffer_wr_clear;
    ...
    logic buffer_overflow_error_wr;
    logic buffer_overflow_error_meta;
    ...
    assign mode_error        = (in_valid && !mode_supported)
                             ...
                             || (buffered_mode && in_valid && !buffer_in_ready)
    ...
    assign parallel_core_in_valid = parallel_mode ? in_valid
                                  ...
                                  : 1'b0;
    assign parallel_core_active_lanes = buffered_mode ? ACTIVE_LANES_W'(PARALLEL_FACTOR)
    ...
    always_ff @(posedge sample_clk or negedge sample_reset_n) begin
        if (!sample_reset_n) begin
            buffer_wr_clear_meta <= 1'b1;
            buffer_wr_clear      <= 1'b1;
        end else begin
            buffer_wr_clear_meta <= buffer_clear;
            buffer_wr_clear      <= buffer_wr_clear_meta;
        end
    end

    always_ff @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            buffer_overflow_error_meta <= 1'b0;
            buffer_overflow_error      <= 1'b0;
        end else begin
            buffer_overflow_error_meta <= buffer_overflow_error_wr;
            buffer_overflow_error      <= buffer_overflow_error_meta;
        end
    end
    ...
    l1_08_v2_input_buffer #(
        ...
        .wr_clear(buffer_wr_clear),
        .in_i(x_i),
        .in_q(x_q),
        .in_valid(in_valid && !buffer_wr_clear),
        .in_ready(buffer_in_ready),
        .rd_clk(clk),
        .rd_reset_n(reset_n),
        .rd_clear(buffer_clear),
        .out_i(buffer_out_i),
        .out_q(buffer_out_q),
        .out_valid(buffer_out_valid),
        .out_ready(buffer_out_ready),
        .overflow_error(buffer_overflow_error_wr)
    );
